...
#	NAME:		veraview.py
#	HISTORY:
#	    2015-06-13  william.gurecky@gmail.com
#	  New VeraViewNotebook and VeraViewTab classes
...
try:
    import wx
    if '3.0.2' not in wx.version():
        raise ImportError
    import wx.lib.delayedresult as wxlibdr
except Exception:
    raise ImportError(
        'The wxPython module v3.0.2 is required to run this program')
...
TITLE = 'VeraOutputViewer (Build 12)'
    ...
    def OnEventLoopEnter(self, *args, **kwargs):
        ...
        # else:
            ...
            if self.filepath is not None and os.path.exists(self.filepath):
                self.frame.notebook.newOpenFile(self.filepath)
            else:
                self.frame.notebook._OnOpenFile(None)
...
#	CLASS:		VeraViewTab
#------------------------------------------------------------------------
class VeraViewTab(wx.Panel):
    """
    Contains VeraViewGrid
    VeraViewGrid contains widgets.
    """
...
#		--
    ...
    """
        ...
        try:
            ...
            # widget_count = len( self.grid.GetChildren() ) - 1  # - 2 + 1 (new
            # one)
...
#        self.axialBean.axialLevel = self.state.axialLevel
    ...
    #----------------------------------------------------------------------
    def _OnNew(self, ev):
        print >> sys.stderr, '[VeraViewTab._OnNew]'
        ev.Skip()

        #title = None
        menu = ev.GetEventObject()
        item = menu.FindItemById(ev.GetId())
        if item is not None and item.GetText() in WIDGET_MAP:
...
#      self.CreateWidget( WIDGET_MAP[ title ] )
    ...
    def _OnWidgetTool(self, ev):
        print >> sys.stderr, '[VeraViewTab._OnWidgetTool]'
        ev.Skip()
    ...
    def LoadDataModel(self, file_path):
        ...
        Must be called from the UI thread.
        """
        print >> sys.stderr, '[VeraViewTab.LoadDataModel]'
...
#        for ds in ds_names:
#            item = wx.MenuItem(
#                self.timeDataSetMenu, wx.ID_ANY, ds, kind=wx.ITEM_RADIO)
...
#		--
        if len(data.GetDataSetNames()['detector']) > 0:
            ...
            if data.core.ndetax > 1:
                widget_list.append(
                    'widget.detector_axial_plot.DetectorAxialPlot')
...
#      widget_list.append( 'widget.exposure_plot.ExposurePlot' )
        ...
        #widget_list = [ 'widget.time_plot.TimePlot' ]
...
#		--
        ...
        # TODO: Set tab tile to something more appropriate
        #self.SetTitle(title)
...
#  #end _OnCloseWindow
    ...
    def _OnControlTool(self, ev):
        print >> sys.stderr, '[VeraViewTab._OnControlTool]'
        ev.Skip()
    ...
    def _OnGridResize(self, ev):
        ...
        if new_size is not None and new_size != cur_size:
            ...
            if widget_count > (new_size[0] * new_size[1]):
                ...
                wx.MessageDialog(
                    self, message, 'Resize Grid').ShowWindowModal()
...
class VeraViewNotebook(wx.Notebook):
    """
    Contains TabPanels
    """
    def __init__(self, parent, state):
        wx.Notebook.__init__(self, parent, id=wx.ID_ANY, style=wx.BK_DEFAULT)
        self.tab = []  # Stores tabs
        self.n = 0
        self.state = state
        self.tabCount = -1
    ...
    def onButtonRemove(self, event):
        ...
        if len(self.tab) == 0:
            ...
            pass
        try:
            self.currentTab[0].CloseAllWidgets()
        except:
            pass
        self.DeletePage(self.currentTab[1])
        self.SendSizeEvent()
        del self.tab[self.currentTab[1]]
        self.tabCount -= 1

    def AddNewPage(self, n, tabName):
        """
        Construct Tab
        """
        self.tab.append(VeraViewTab(self, self.state))
        self.AddPage(self.tab[n], tabName, select=True)
        self.currentTab = [self.tab[n], n]

    def OnPageChanged(self, event):
        # old = event.GetOldSelection()
        # sel = self.GetSelection()
        self.currentTab = [self.tab[event.GetSelection()], event.GetSelection()]
    ...
    def _OnWidgetTool(self, ev):
        ...
        """
        self.currentTab[0]._OnWidgetTool(ev)

    def _OnGridResize(self, ev):
        """
        Apply to current tab
        """
        self.currentTab[0]._OnGridResize(ev)
    ...
    def _OnOpenFile(self, ev):
        ...
        if dialog.ShowModal() != wx.ID_CANCEL:
            path = dialog.GetPath()
            dialog.Destroy()
            self.AddNewPage(self.tabCount + 1, "..." + str(path[-20:]))
            self.tab[self.tabCount + 1]._OnOpenFile(path, ev)
            self.tabCount += 1

    def _OnNew(self, ev):
        """
        Generates a new widget window in current tab
        """
        self.currentTab[0]._OnNew(ev)

    def newOpenFile(self, path):
        """
        Open file from cmd line supplied path
        """
        try:
            self.AddNewPage(self.tabCount + 1, "..." + str(path[-20:]))
            self.tab[self.tabCount + 1]._OnOpenFile(path, None)
            self.tabCount += 1
        except:
            pass
...
class VeraViewFrame(wx.Frame):
    ...
    Contains Menu Bar.
    Contains clickable widget launchers.
    Contains VeraViewNotebook.
    """
#		-- Object Methods
#		--
    #----------------------------------------------------------------------
    ...
    def _InitUI(self):
        ...
        inside_panel = wx.Panel(self)
        self.notebook = VeraViewNotebook(self, self.state)
...
#			-- File->New Submenu
        ...
        for k in widget_keys:
            ...
            self.Bind(wx.EVT_MENU, self.notebook._OnNew, item)
            new_menu.AppendItem(item)
        file_menu.AppendSubMenu(new_menu, '&New')

#			-- Open Item
        open_item = wx.MenuItem(file_menu, wx.ID_ANY, '&Open\tCtrl+O')
        self.Bind(wx.EVT_MENU, self.notebook._OnOpenFile, open_item)
        file_menu.AppendItem(open_item)

        min_item = wx.MenuItem(
            file_menu, wx.ID_ANY, '&Minimize Window\tCtrl+M')
...
#    self.Bind( wx.EVT_MENU, self.notebook._OnSaveWindow, save_im_item )
...
#		--
        ...
        self.Bind(wx.EVT_MENU, self.notebook._OnGridResize, grid_item)
...
#		--
        ...
        for ti in TOOLBAR_ITEMS:
            ...
            widget_tbar.AddTool(
                int(ti_count), widget_im.ConvertToBitmap(), shortHelpString=widget_name)
            self.Bind(wx.EVT_TOOL, self.notebook._OnWidgetTool, id=ti_count)
            ti_count += 1
        # end for
        widget_tbar.Realize()
...
#       -- Remove tab button
#
        self.buttonRemoveTab = wx.Button(self, id=wx.ID_ANY, label="Close Current Tab", size=(225, 22))
        self.buttonRemoveTab.Bind(wx.EVT_BUTTON, self.notebook.onButtonRemove)

#       -- Define Notebook Sizer
#		--
        notebook_sizer = wx.BoxSizer(wx.VERTICAL)
        notebook_sizer.Add(widget_tbar, 0, wx.EXPAND, 0)
        notebook_sizer.Add(self.buttonRemoveTab)
        notebook_sizer.Add(self.notebook, 1, wx.ALL | wx.EXPAND, 5)
            ...
            for child in self.GetChildren():
                if isinstance(child, WidgetContainer):
                    try:
                        ...
                        print >> sys.stderr, '[VeraViewGrid.FireStateChanged]', str(
                            ex)
            # end for
