...
#         Revised back to averages only in derived menu.  Also using the
#         new mechanism for deriving in DataModel.ResolveDerivedDataSet().
#               2019-01-17      leerw@ornl.gov                          -
#         Added special hooks or 'fluence'.
#               2019-01-02      leerw@ornl.gov                          -
#         Adding "Set Visible Datasets..." item to DataSetsMenu.
#               2018-11-07      leerw@ornl.gov                          -
...
from .dataset_list_bean import *
from .datamodel_mgr_tree import *
...
#               --
  ...
  def __init__( self,
    ...
    Args:
        state (event.State): required object instance
        binder (wx.Window): required object from window containing this menu
            on which to call Bind() for menu events
        data_model (data.datamodel.DataModel): required model for this menu
        mode (str): mode value ('selected' implies 'multi')
            ...
            'subselected': multiple selections with "Selected xxx" items and
                per-type submenus (NOT YET IMPLEMENTED)
            'subsingle'    single selection, per-type submenus (**tested**)
        ds_listener (object): for multiple selections, this object will be
            called on methods {Is,Toggle}DataSetVisible();
            for single selections STATE_CHANGE_curDataSet is fired
        ds_types (list(str)): defined allowed data types, where None means
            all types in ``data_model``
        show_core_datasets (bool): True to show a core submenu
        show_derived_menu (bool): True to show a derived submenu if applicable
        #special_type (str): special type passed to Widget.GetCurDataSet()
        widget (widget.Widget): widget if used for a widget
"""
...
#               -- Types are resolved in _LoadDataModel()
    ...
    #self.specialDataType = special_type
    self.state = state
    self.widget = widget
  #end __init__
...
"""
  ...
  #----------------------------------------------------------------------
  #     METHOD:         DataModelMenu._GetFluenceDataSet()              -
  #----------------------------------------------------------------------
  def _GetFluenceDataSet( self ):
    """If self.isSingleSelection and self.widget exists, returns that value,
otherwise returns self.state.fluenceAddr.dataSetName
"""
    if self.IsSingleSelection() and self.widget is not None:
      qds_name = self.widget.GetCurDataSet( 'fluence' )
    else:
      qds_name = self.state.GetFluenceAddr().GetDataSetName()
    return  qds_name
  #end _GetFluenceDataSet
...
#        types_in.remove( 'tally' )
  ...
  def _OnDataSetListItem( self, qds_names, ev ):
    ...
    if dialog.ShowModal() != wx.ID_CANCEL:
      ...
      if len( selections ) > 0:
        qds_name = selections[ 0 ]
        if self.IsSingleSelection():
          if self.widget is not None:
            #self.widget.SetDataSet( qds_name )
            self.binder.GetTopLevelParent().GetApp().\
                DoBusyEventOp( self.widget.SetDataSet, qds_name )
          else:
            ds_type = self.dataModel.GetDataSetType( qds_name.modelName )
            if ds_type == 'fluence':
              reason = self.state.Change( None, fluence_dataset = qds_name )
            else:
              reason = self.state.Change( None, cur_dataset = qds_name )
            #self.state.FireStateChange( reason )
            self.binder.GetTopLevelParent().GetApp().\
                DoBusyEventOp( self.state.FireStateChange, reason )
        elif listener:
  ...
  #----------------------------------------------------------------------
  def _OnDerivedDataSetListItem( self, der_label, names, ev ):
    ev.Skip()
    ...
    if dialog.ShowModal() != wx.ID_CANCEL:
      ...
      if len( selections ) > 0:
        ...
        if der_ds_name:
          der_qds_name = DataSetName( self.dataModel.name, der_ds_name )

          if self.IsSingleSelection():
            if self.widget is not None:
              #self.widget.SetDataSet( der_qds_name )
              self.binder.GetTopLevelParent().GetApp().\
                  DoBusyEventOp( self.widget.SetDataSet, der_qds_name )
            else:
  ...
  #----------------------------------------------------------------------
  def _OnDerivedDataSetMenuItem( self, der_label, ev ):
    ev.Skip()
...
#            self.state.FireStateChange( reason )
      ...
      self.logger.exception(
          'handling derived menu item, widget=%s', self.widget
          )
      wx.MessageBox(
...
#      self.UpdateMenu()
    ...
    if (reason & STATE_CHANGE_fluenceAddr) > 0:
      if self.IsSingleSelection() and self.dataModel:
        fluence_addr = self.state.GetFluenceAddr()
        item = self._FindMenuItem( 'fluence', fluence_addr.dataSetName )
        if item:
          self._CheckSingleItem( item )
      #end if single selection
    #end if fluenceAddr
...
#		-- of derivable datasets
    ...
    for ds_type in self.dataSetTypes:
      #xxxxx For now omitting ':node'
      #if self.dataModel.IsDerivedType( ds_type ):
      if self.dataModel.IsDerivedType( ds_type ) and ds_type != ':node':
	...
	for base_type in self.dataModel.GetDerivableTypes( der_label ):
	  base_ds_names = self.dataModel.GetDataSetNames( base_type )
          names = [
              v for v in base_ds_names
              if not self.dataModel.IsCoreGroupDataSet( v )
              ]
	  ds_names += names
...
#		--
	  ...
	  for ds_name in sorted( ds_names ):
	    ...
	    if self.dataModel.HasDerivedDataSet( ds_type, der_label, ds_name ):
	      ...
	      self.binder.Bind(
                  wx.EVT_MENU,
                  functools.
                      partial( self._OnDerivedDataSetMenuItem, der_label ),
                  item
                  )
          ...
          if list_item is not None:
            self.binder.Bind(
                wx.EVT_MENU,
                functools.partial(
                    self._OnDerivedDataSetListItem, der_label, list_names
                    ),
                list_item
                )
  ...
  #----------------------------------------------------------------------
  #     METHOD:         DataModelMenu._UpdateDerivedMenu_aggs()         -
  #----------------------------------------------------------------------
  def _UpdateDerivedMenu_aggs( self ):
    """
"""
#               -- Clear existing items
#               --
...
#               -- Build map by derived label of maps by agg function
#               -- of derivable datasets
    derived_menu_map = {}
    for ds_type in self.dataSetTypes:
      #xxxxx For now excluding ':node'
      #if self.dataModel.IsDerivedType( ds_type ):
      if self.dataModel.IsDerivedType( ds_type ) and ds_type != ':node':
        der_label = self.dataModel.GetDataSetTypeDisplayName( ds_type )
        agg_base_map = self.dataModel.GetDerivableFuncsAndTypes( der_label )

        aggs_menu_map = derived_menu_map.get( der_label )
        if aggs_menu_map is None:
          derived_menu_map[ der_label ] = aggs_menu_map = {}

        for agg_name, base_types in six.iteritems( agg_base_map ):
          ds_names = aggs_menu_map.get( agg_name )
          if ds_names is None:
            aggs_menu_map[ agg_name ] = ds_names = []
          for base_type in base_types:
            ds_names += self.dataModel.GetDataSetNames2( base_type, True )
        #end for agg_name, base_types in six.iteritems( agg_base_map )
      #end if ds_type is derived
    #end for ds_type

#               -- 1st level pullrights for derived type label
#               --
    item_kind = wx.ITEM_NORMAL
    for der_label, aggs_menu_map in sorted( six.iteritems( derived_menu_map ) ):
      der_label_menu = wx.Menu()

      #agg_menu = wx.Menu()

#                       -- 2nd level is agg function
      for agg_name, ds_names in sorted( six.iteritems( aggs_menu_map ) ):
        if len( ds_names ) > 0:
          agg_menu = wx.Menu()
...
#                               -- 3rd level is source dataset name
          ...
          agg_item = wx.MenuItem(
              der_label_menu, wx.ID_ANY, agg_name,
              subMenu = agg_menu
              )
          der_label_menu.AppendItem( agg_item )
        #end if len( ds_names ) > 0
      #end for agg_name, ds_names in sorted( six.iteritems( aggs_menu_map ) )
      ...
      self.derivedMenu.AppendItem( der_label_item )
    #end for der_label, aggs_menu_map in sorted( six.iteritems( derived_menu_map ) )
  #end _UpdateDerivedMenu_aggs
...
"""
    ...
    if bool( self ) and self.dataModel is not None and \
...
#                                       -- Create per-type menus
          ...
          cur_dataset = self._GetCurDataSet()
          for dtype, dataset_names in sorted( names_by_type.iteritems() ):
            dtype_menu = wx.Menu()
            cur_selection = \
                self._GetFluenceDataSet()  if dtype == 'fluence' else \
                cur_dataset
            list_item = None
...
#                               -- Flat
          ...
          #cur_selection = self._GetCurDataSet()
          cur_selection = \
              self._GetFluenceDataSet() \
              if self.dataSetTypes == [ 'fluence' ] else \
              self._GetCurDataSet()
...
#               --
  ...
  def __init__( self,
    ...
    """Initializes with an empty menu.
    Args:
        state (event.state.State): State instance, required
        binder (wx.Window): object from window containing this on which to
            call ``Bind()`` for menu events
        mode (str): mode value ('selected' implies 'multi')
            ''             single selection, flat menu (default)
            'multi'        multiple selections, flat menu
            'selected'     multiple selections with "Selected xxx" items,
            ...
            'subsingle'    single selection, per-type submenus (**tested**)
        ds_listener (object):  for multiple selections, this object will be
            called on methods ``IsDataSetVisible()`` and
            ``ToggleDataSetVisible()``; for single selections
            ``STATE_CHANGE_curDataSet`` is fired
        ds_types (list):  defined allowed types, where None means all types
            in the data model
        show_core_datasets (bool):  True to show a core submenu
        show_derived_menu (bool):  True to show a derived submenu if
            applicable
        show_dialog_item (bool):  True to show an item for toggling visibility
            of multiple datasets
        #special_type (str): special type passed to Widget.GetCurDataSet()
        widget (widget.Widget):  Widget instance for use in a widget
"""
    super( DataSetsMenu, self ).__init__(
        state, binder,
        data_model = None, mode = mode,
        ds_listener = ds_listener, ds_types = ds_types,
        show_core_datasets = show_core_datasets,
        show_derived_menu = show_derived_menu,
    ...
    self.showDialogItem = show_dialog_item
...
#         update_all = False
  ...
  #----------------------------------------------------------------------
  #	METHOD:		_OnVisibleDataSetsItem()                        -
  #----------------------------------------------------------------------
  def _OnVisibleDataSetsItem( self, ev ):
    """
"""
    ev.Skip()
...
"""
    ...
    if (reason & STATE_CHANGE_fluenceAddr) > 0:
      if 'self' in self.modelSubMenus:
        self.ProcessStateChange( STATE_CHANGE_fluenceAddr )
      else:
        for ds_menu in self.modelSubMenus.values():
          ds_menu.ProcessStateChange( STATE_CHANGE_fluenceAddr )
    #end if fluenceAddr
  #end OnStateChange
...
"""
    ...
    if dmgr is not None:
      ...
      if self.showDialogItem and self.widget is not None:
        dialog_item = wx.MenuItem(
            self, wx.ID_ANY, 'Set Visible Datasets ...',
            kind = wx.ITEM_NORMAL
            )
        self.AppendItem( dialog_item )
        if bool( self.binder ):
          self.binder.\
              Bind( wx.EVT_MENU, self._OnVisibleDataSetsItem, dialog_item )
    #end if dmgr
  #end UpdateAllMenus
